...
def my_worksheet(rol, documento, hoja, row = None, col = None):
    ...
    if rol == 'servicio':
        if row == None:
            PATH_PRINCIPAL = os.path.join('home','Analisis','basic-flask-app',"src","reportes","security","client_desktop.json")
            
            # .oauth(credentials_filename=
            sa = gspread.oauth(credentials_filename=PATH_PRINCIPAL)
